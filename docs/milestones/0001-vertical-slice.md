...
1. **Room/role types + store.** Drizzle migration for `rooms`, `roles`, `role_grants`. Seed `party-1` and `admin-1` with `player` and `gm` roles at boot.
2. **Scope-filtered retrieval.** `retrieve(roomId, roleId, query?) → Statement[]`. Enforces read-set at the store. *Test: scope-isolation (admin reads see no party content without oversight).*
3. **Embeddings + vector search.** Approved plan: introduce an `Embedder` interface (`src/core/embedder.ts`) and a `SearchBackend` interface (`src/core/search.ts`; `SearchResult = { statementId, score, chunkIndex? }` — `chunkIndex?` is forward-compatible for a future chunked-statement schema). Ship one v1 `Embedder` impl — `HashEmbedder` (`src/store/embedders/hash.ts`): lowercase, whitespace-split, FNV-1a per token into a `env.EMBED_DIM`-wide bag, L2-normalize. **No API-key dependency in this task** — provider-backed embedders (OpenAI, Voyage, …) arrive later via Mastra's provider layer; the `Embedder` interface is the seam. Ship one v1 `SearchBackend` impl — `PgvectorSearchBackend` (`src/store/search/pgvector.ts`): constructor takes an `Embedder`; `search(patterns, {text, limit})` reuses `retrieval.ts::patternToSql` for scope WHERE, adds `AND embedding IS NOT NULL`, `ORDER BY embedding <=> $vec` LIMIT. Add `src/store/vectors.ts` with module-level singletons (`get/setEmbedder`, `get/setBackend`) plus `appendAndIndex(input)` — computes embedding, on failure logs warn and proceeds with null embedding, warns above `env.LONG_CONTENT_WARN_CHARS` (default 6000), then `appendStatement({...input, embedding})`. Add IVFFlat cosine index in `migrate.ts` (`CREATE INDEX IF NOT EXISTS statements_embedding_idx ON statements USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)`). Extend `retrieval.ts`: add `score?: number` to `StatementRow`; when `opts.query` is set in `retrieveForUserRoom` / `retrieveByScopes`, call `getBackend().search(patterns, {text: query, limit})` then hydrate rows by id and attach scores; export `patternToSql`. Replace the hand-crafted vector in `src/store/smoke.ts` with an `appendAndIndex` round-trip + query-based retrieval assertion. Add `LONG_CONTENT_WARN_CHARS` to `src/config/env.ts`. **Do not modify `src/models/registry.ts`** — embedding-provider resolution is deferred. *Tests (`test/integration/vector-search.test.ts`):* seed party/world/admin statements with overlapping content (e.g. "sword" variants) and one unembedded party row; assert ANN-for-"sword" as PLAYER_USER returns world+party rows in similarity order and excludes admin; ANN-for-"dragon" ranks party dragon first; unembedded row never appears; chronological path unaffected; long-content warn fires above threshold. Full plan was saved at `/home/alan/.claude/plans/memoized-tickling-locket.md`.
