...
5. **Model registry.** ✅ Done. `resolveModel("<provider>:<slug>")` per D49 — Anthropic + OpenAI wired; OpenRouter when first needed. _Test: model-registry (9 unit cases, all green)._
6. **Resolver agent framework + dnd5e skill-check.** ✅ Done. `AgentBackedResolver` — generic context-driven resolver parameterized by `(systemId, modelSpec, instructions)`, not hardcoded per-system. Instructions are markdown (data, not code); dnd5e skill-check is an example instructions file. Shared tool primitives: `roll(count, sides, modifier?, seed?)`, `retrieve(scope, query)`. Uses `generateObject` with structured `ResolveResult` output. `DeterministicResolver` and `HybridResolver` are future implementations of the same `Resolver` interface. _Test: deterministic with mock provider + seeded roll._
7. **Narrator agent.** ✅ Done. Reads slice → composes → emits `narration`/`pose`/`invention`. Invention auto-emits `open-question` routed to `gm`. _Test: narrator round-trip; invention produces open-question in admin scope._
8. **Live-responder worker.** ✅ Done. Event-triggered on new user statement in a party room; invokes narrator; invokes resolver if mechanical action detected.
9. **Steering-formalizer agent + open-question-resolver worker.** ✅ Done. GM freeform decision → `authoring-decision` → resolver applies (promote / reject / supersede). `SteeringFormalizer` agent takes subject/candidate explicitly (caller fetches OQ); `openQuestionResolverWorker` triggered on `authoring-decision` in governance scope, applies decision via supersedes chain. Also fixed a `vectors.ts` circular-dep (see D53). _Test: open-question-flow (7 integration cases, all green)._
...
13. **HTTP API surface + hermetic tests.** ✅ Done. Hono app with `/health` + statement CRUD (`POST /api/statements`, `GET /api/statements/:id`, `GET /api/statements`). Python + pytest + httpx black-box test suite in `test-api/`. Docker Compose for hermetic stack. See `docs/plan-api-surface-and-hermetic-tests.md`.
