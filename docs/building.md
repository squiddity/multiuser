...
For demo/test methodology, confirm the configured provider is available before judging narrative behavior. The current default model for demos and bot runs is `openrouter:nvidia/llama-3.3-nemotron-super-49b-v1.5`. The preferred hosted fallback is `openrouter:openai/gpt-4o-mini`. If a hosted provider returns empty text, times out, or is quota-limited, switch `DEFAULT_MODEL_SPEC` to the fallback and rerun the demo; classify provider/runtime failures separately from application behavior.
